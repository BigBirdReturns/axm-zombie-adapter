...
def main():
    ...
    p_plan = sub.add_parser("plan", help="Generate plan.json from cluster manifest YAML")
    ...
    p_exp.add_argument("target", choices=["exo", "petals"], help="Exporter target")
    ...
    if args.cmd == "export":
        ...
        if args.target == "exo":
            export_exo(plan, outdir)
        else:
            export_petals(plan, outdir)
